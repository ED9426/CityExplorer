...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
   ...
   * that are introduced by inserting new nodes into a red black tree and
   * ensure that the worst-case height of the tree remains O(log n), where n is
   * the size or number of elements stored in the red black tree.
   */
  private void enforceRBTreePropertiesAfterInsert(Node<T> newNode) {

    if (newNode.parent != null
        && !newNode.parent.isBlack) {
      // scenario 1: parent's sibling is black and opposing from newNode
      if ((newNode.data.compareTo(newNode.parent.data) > 0
          && newNode.data.compareTo(newNode.parent.parent.data) > 0
        ...
        // swap colors of two nodes that will be rotated
        boolean temp;
        temp = newNode.parent.isBlack;
        newNode.parent.isBlack = newNode.parent.parent.isBlack;
        newNode.parent.parent.isBlack = temp;
        // rotate the new node's parent and grandparent
        rotate(newNode.parent, newNode.parent.parent);

      }

      // scenario 2: parent's sibling is black and not opposing from newNode
        ...
        // rotate two violating red nodes
        rotate(newNode, newNode.parent);
        // swap colors of two nodes that will be rotated below
        boolean temp;
        temp = newNode.isBlack;
        newNode.isBlack = newNode.parent.isBlack;
        newNode.parent.isBlack = temp;
        // rotate the new node and its parent
        rotate(newNode, newNode.parent);

      }

      // scenario 3: parent's sibling is red
      else if (!newNode.parent.parent.leftChild.isBlack
          && !newNode.parent.parent.rightChild.isBlack) {

        // change new node's grandparents to red
        newNode.parent.parent.isBlack = false;
        // change new node's parent and sibling to black
        newNode.parent.parent.leftChild.isBlack = true;
        newNode.parent.parent.rightChild.isBlack = true;
        // check if there's violation for red grandparent
  ...
  public void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    ...
    if (parent.rightChild == child) { // left rotation
      if (parent.parent != null) { // if parent is not the root
      ...
      parent.parent = child;
    } else if (parent.leftChild == child) { // right rotation
      if (parent.parent != null) { // if parent is not the root
        if (parent.parent.leftChild == parent) {
          parent.parent.leftChild = child;
        } else {
      ...
      throw new IllegalArgumentException("The child is not a child of the parent.");
    }
  }
